...
### LINEログインフロー
1. プロフィール画面の「LINEでログイン」タップ → `/api/auth/line` へ遷移
2. LINE認証 → コールバックURL: `https://figma-importer.replit.app/api/auth/callback/line`
3. サーバーがLINEプロフィール（名前・アイコン）取得、DB保存、JWT発行
4. `/?line_token=JWT` へリダイレクト
5. `app/_layout.tsx` の `LineTokenHandler` がトークンを検出してASyncStorageに保存 → マイページへ遷移
- Channel ID: `LINE_CHANNEL_ID` 環境変数
- Channel Secret: `LINE_CHANNEL_SECRET` 環境変数
- Callback URL: `https://figma-importer.replit.app/api/auth/callback/line`
